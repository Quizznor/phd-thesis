...
class Architectures:
    ...
    if True:  # so that this can be collapsed in editor =)
        ...
        def add_lstm(**kwargs) -> None:
            ...
            dense_outputs = [
                tf.keras.layers.LSTM(kwargs.get("d_LSTM", 1), activation="relu")(x)
                for x in unstacked
            ]
            merged = tf.keras.layers.Lambda(lambda x: tf.stack(x, axis=1))(
                dense_outputs
            )
            merged_flatten = tf.keras.layers.Flatten()(merged)

            return tf.keras.Model(
                input, tf.keras.layers.Dense(2, activation="softmax")(merged_flatten)
            )
...
class NNClassifier(Classifier):
    ...
    def __init__(
        self, name: str, set_architecture=None, supress_print: bool = False
    ) -> None:
        r"""
        ...
        if set_architecture is None:
            ...
            try:
                self.model = tf.keras.models.load_model(
                    "/cr/data01/filip/models/" + name + "/model_converged"
                )
                ...
                if len(available_models) == 0:
                    ...
                    choice = available_models[0]
                self.model = tf.keras.models.load_model(
                    "/cr/data01/filip/models/" + name + "/" + choice
                )
    ...
    def train(self, Datasets: tuple, epochs: int, **kwargs) -> None:
        ...
        early_stopping_patience = kwargs.get(
            "early_stopping_patience", int(0.5 * len(Datasets[0]))
        )
        early_stopping_accuracy = kwargs.get(
            "early_stopping_accuracy", GLOBAL.early_stopping_accuracy
        )

        EarlyStopping = BatchwiseEarlyStopping(
            early_stopping_patience, early_stopping_accuracy
        )
        ...
        with open(
            f"/cr/data01/filip/models/{self.name}/model_{self.epochs}/training_files.csv",
            "w",
        ) as training_file:
        ...
        with open(
            f"/cr/data01/filip/models/{self.name}/model_{self.epochs}/validation_files.csv",
            "w",
        ) as validation_file:
        ...
        f, df, n, n_trig, t = self.production_test(
            30000,
            apply_downsampling=kwargs.get("apply_downsampling", False),
            window_length=window_length,
            window_step=window_step,
        )

        with open(
            f"/cr/data01/filip/models/{self.name}/model_{self.epochs}/production_test.csv",
            "w",
        ) as random_file:
            random_file.write(
                "# f  df  n_traces  n_total_triggered  total_trace_duration\n"
            )
    ...
    def get_files(self, dataset: str) -> typing.Union[list, None]:
        ...
        return list(
            np.loadtxt(
                f"/cr/data01/filip/models/{self.name}/model_{self.epochs}/{dataset}_files.csv",
                dtype=str,
            )
        )
