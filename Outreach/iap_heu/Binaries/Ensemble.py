...
class Ensemble(NNClassifier):
    ...
    def __init__(
        self,
        name: str,
        set_architecture: str = None,
        n_models: int = GLOBAL.n_ensembles,
        **kwargs,
    ) -> None:
        r"""
        ...
        * *early_stopping_patience* (``int``) -- number of batches without improvement before training is stopped
        ...
        for i in range(1, n_models + 1):
            ...
            try:
                ThisModel = NNClassifier(
                    "ENSEMBLES/" + name + f"/ensemble_{str(i).zfill(2)}/",
                    set_architecture,
                    supress_print,
                )
        ...
        not kwargs.get("supress_print", False) and print(
            f"{self.name}: {n_models} models successfully initiated\n"
        )
        if n_fail:
            print(f"{n_fail} models couldn't be initialized!")
    ...
    def money_plot(self, dataset: str) -> None:
        ...
        if not os.path.isdir(
            f"/cr/users/filip/MoneyPlot/data/{self.name.split('/')[-1][9:]}"
        ):
            os.makedirs(
                f"/cr/users/filip/MoneyPlot/data/{self.name.split('/')[-1][9:]}"
            )
        ...
        np.savetxt(
            f"/cr/users/filip/MoneyPlot/data/{self.name.split('/')[-1][9:]}/{dataset}.csv",
            save_matrix,
        )
    ...
    def get_best_model(self, dataset: str) -> NNClassifier:
        ...
        try:
            acc, acc_err, rate, rate_err, true_acc = np.loadtxt(
                f"/cr/users/filip/MoneyPlot/data/{self.name[9:]}/{dataset}.csv",
                unpack=True,
            )

            SN_ratios = (
                rate / true_acc
            )  # this is technically a noise to signal ratio, hence we must MINIMIZE
