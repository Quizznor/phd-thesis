...
class RandomTraceError(Exception):
    pass


class ElectronicsError(Exception):
    pass
...
def get_fit_function(root_path: str, e: int, t: int) -> np.ndarray:
    ...
    theta_range = {0: "0_33", 1: "33_44", 2: "44_51", 3: "51_56", 4: "56_65"}
    ...
    try:
        ...
        fit_parameters = np.array([0.0, 0.0, 1.0])
...
def lateral_trigger_probability(
    x: np.ndarray, efficiency: float, prob_50: float, scale: float
) -> np.ndarray:
...
def lateral_trigger_probability_error(
    x: np.ndarray,
    pcov: np.ndarray,
    efficiency: float,
    prob_50: float,
...
def lateral_distribution_function(
    x: np.ndarray, efficiency: float, prob_50: float, scale: float
) -> np.ndarray:
    return np.clip(efficiency * (1 - 1 / (1 + np.exp(-(x - prob_50) / scale))), 0, 1)


def lateral_distribution_function_error(
    x: np.ndarray, pcov: np.ndarray, efficiency: float, prob_50: float, scale: float
) -> np.ndarray:

    errors = np.zeros_like(x)

    d_eff = (
        lambda u: lateral_distribution_function(u, efficiency, prob_50, scale)
        / efficiency
    )
    d_prob_50 = (
        lambda u: efficiency
        * scale
        * np.exp(scale * (u + prob_50))
        / (np.exp(prob_50 * scale) + np.exp(scale * u)) ** 2
    )
    d_scale = (
        lambda u: efficiency
        * (prob_50 - u)
        * np.exp(scale * (prob_50 + u))
        / (np.exp(prob_50 * scale) + np.exp(scale * u)) ** 2
    )
...
def progress_bar(current_step: int, total_steps: int, start_time: int) -> None:
    ...
    eta = strftime(
        "%H:%M:%S",
        gmtime(time_spent * (total_steps - current_step) / (current_step + 1)),
    )
    percentage = int((current_step + 1) / total_steps * 100)

    print(
        f"Step {current_step + 1}/{total_steps} | {elapsed} elapsed ||{'-' * (percentage // 5):20}|| {percentage}% -- {ms_per_iteration:.3f} ms/step, ETA: {eta}",
        end="\r",
    )
