...
class HardwareClassifier(Classifier):
    ...
    def __init__(self, triggers: list = ["th2", "tot", "totd"], name: str = False):
        ...
        if "th1" in triggers:
            self.triggers.append(self.Th1)
        if "th2" in triggers:
            self.triggers.append(self.Th2)
        if "tot" in triggers:
            self.triggers.append(self.ToT)
        if "totd" in triggers:
            self.triggers.append(self.ToTd)
        if "mops" in triggers:
            self.triggers.append(self.MoPS_compatibility)
    ...
    def __call__(self, trace: typing.Union[np.ndarray, Trace]) -> bool:
        ...
        if isinstance(trace, Trace):
            ...
            if self.Th2 in self.triggers or self.Th1 in self.triggers:
                ...
                if self.Th1 in self.triggers and self.Th1(full_trace):
                    return True
                if self.Th2 in self.triggers and self.Th2(full_trace):
                    return True

            # check for rest of the triggers in sliding window analysis
            rest_of_the_triggers = [
                trigger
                for trigger in self.triggers
                if trigger not in [self.Th1, self.Th2]
            ]

            if rest_of_the_triggers == []:
                return False
    ...
    def MoPS_compatibility(signal: np.ndarray) -> bool:
        ...
        for pmt in signal:
            ...
            if sum(pmt) * GLOBAL.q_peak <= integral_check:
                continue

            occupancy = 0
            positive_steps = np.nonzero(np.diff(pmt) > 0)[0]
            steps_isolated = np.split(
                positive_steps, np.where(np.diff(positive_steps) != 1)[0] + 1
            )
            candidate_flanks = [
                step for step in steps_isolated if len(step) >= min_slope_length
            ]
            candidate_flanks = [
                np.append(flank, flank[-1] + 1) for flank in candidate_flanks
            ]
            ...
            for i, flank in enumerate(candidate_flanks):
                ...
                for j, consecutive_flank in enumerate(candidate_flanks[i + 1 :], 0):
                    ...
                    if n_continue_at_index <= consecutive_flank[0]:
                        break  # no overlap, no need to do anything
                    elif (
                        consecutive_flank[0]
                        < n_continue_at_index
                        <= consecutive_flank[-1]
                    ):  # partial overlap, adjust next flank
                        overlap_index = np.argmin(
                            np.abs(consecutive_flank - n_continue_at_index)
                        )
                        candidate_flanks[i + j] = candidate_flanks[i + j][
                            overlap_index:
                        ]
                        if len(candidate_flanks[i + j]) < min_slope_length:
                            _ = candidate_flanks.pop(i + j)
                        break

                    elif (
                        consecutive_flank[-1] < n_continue_at_index
                    ):  # complete overlap, discard next flank
                        _ = candidate_flanks.pop(i + j)

                if total_y_rise > y_min and total_y_rise < y_max:
                    occupancy += 1
            if occupancy > min_occupancy:
                pmt_active_counter += 1
    ...
    def plot_performance(
        ...
        x, xerr, y, yerr, r_th2, r_tot, r_tod, x_true = np.loadtxt(
            "/cr/data01/filip/models/HardwareClassifier/ROC_curve/money_plot.csv",
            unpack=True,
        )
        ...
        ax.fill_between(
            x_true[scaled_indices],
            y[scaled_indices] - yerr[scaled_indices],
            y[scaled_indices] + yerr[scaled_indices],
            color="k",
            alpha=0.15,
        )
