...
class EventGenerator:
    ...
    def __new__(
        self, datasets: typing.Union[list, str], **kwargs: dict
    ) -> typing.Union[tuple, "EventGenerator"]:
        r"""
        :Keyword arguments:
    ...
    def get_signal_files(user_choice: typing.Union[list[str], str]) -> list[str]:
        ...
        if isinstance(user_choice, str):
            if user_choice in libraries:
                data = [
                    EventGenerator.libraries[user_choice]
                ]  # add single energy library
                ...
                data = [
                    *EventGenerator.libraries.values()
                ]  # add every available library
            elif os.path.isdir(user_choice):
                data = [user_choice]  # add a custom directory

            else:
                raise NotImplementedError(
                    f"input {user_choice} is not supported as argument for 'dataset'"
                )

            all_files = [
                [os.path.abspath(os.path.join(library, p)) for p in os.listdir(library)]
                for library in data
            ]
            all_files = [
                item
        ...
        elif hasattr(user_choice, "epochs"):
            all_files = user_choice.get_files(
                "validation"
            )  # add validation files from classifier
        elif hasattr(user_choice, "models"):
            all_files = user_choice.models[0].get_files(
                "validation"
            )  # add validation files from ensemble

        else:
            raise NotImplementedError(
                f"input {type(user_choice)} is not supported as argument for 'dataset'"
            )
...
class Generator(tf.keras.utils.Sequence):
    ...
    def __init__(self, signal_files: list, **kwargs: dict):
        ...
        * *particle_type* (``list[str]``)   -- what particles to consider in particle cut
        ...
        # Generator options
        self.use_real_background = kwargs.get(
            "real_background", GLOBAL.real_background
        )  # use random trace baselines
        random_index = kwargs.get(
            "random_index", GLOBAL.random_index
        )  # start at this random file
        ...
        # Classifier options
        self.ignore_low_VEM = kwargs.get(
            "ignore_low_vem", GLOBAL.ignore_low_VEM
        )  # integrated signal cut threshold
        self.ignore_particles = kwargs.get(
            "ignore_particles", GLOBAL.ignore_particles
        )  # particle in tank cut threshold
        self.particle_type = kwargs.get(
            "particle_type", GLOBAL.particle_type
        )  # type of particles that count

        if isinstance(self.particle_type, str):
            self.particle_type = [self.particle_type]

        if self.use_real_background:
            self.RandomTraceBuffer = RandomTrace(station=station, index=random_index)

        if self.use_real_background and self.force_inject is None:
            self.force_inject = 0
        ...
        self.__n_sig, self.__n_bkg, self.__n_int, self.__n_prt = (
            np.NaN,
            np.NaN,
            np.NaN,
            np.NaN,
    ...
    def __getitem__(self, index: int) -> typing.Union[tuple[np.ndarray], np.ndarray]:
        ...
        * *for_training = False* -- should ONLY be used during training, returns labelled batches, returns *(Traces, Labels)*
        ...
        if self.prior != 0:
            ...
            try:
                stations = (
                    SignalBatch(self.files[index]) if self.prior != 0 else []
                )  # load this shower file in memory
                full_traces, traces, labels = (
                    [],
                    [],
                    [],
            ...
            for station in stations:
                ...
                baseline = self.build_baseline()
                VEMTrace = Trace(
                    baseline, station, self.trace_options, self.files[index]
                )  # create the trace
                ...
                # where population of SIG and BKG conform to the prior set in __init__
                    ...
                    for window in VEMTrace:
                        ...
                        if this_label == "PRT":
                            ...
                            n_bkg += 1
                            print(
                                f"[WARN] -- invalid {label = } encountered in signal creation!"
                            )
            ...
            if not self.for_training:
                ...
                # more traces than required prior were rejected, delete some "background" traces
                    ...
                    while n_bkg < 0:
                        ...
                        false_backgrounds = np.unique(
                            np.argwhere(labels == self.labels["BKG"])[:, 0]
                        )
    ...
    def __iter__(
        self,
    ) -> typing.Generator[tuple[np.ndarray], np.ndarray, StopIteration]:
    ...
    def get_background_window(self) -> np.ndarray:
        ...
        trace_length = (
            self.trace_length if not self.apply_downsampling else self.trace_length // 3
        )

        if (
            self.BackgroundTrace._iteration_index
            >= trace_length - self.BackgroundTrace.window_length
        ):
        ...
        return self.BackgroundTrace.get_trace_window(
            self.BackgroundTrace._iteration_index - self.BackgroundTrace.window_step
        )
    ...
    def build_baseline(self) -> np.ndarray:
        ...
        if self.use_real_background:
            q_peak, q_charge, baseline = (
                self.RandomTraceBuffer.get()
            )  # load random trace baseline
            baseline += np.random.uniform(
                0, 1, size=baseline.shape
            )  # convert int ADC to float ADC
            ...
            baseline = Baseline(
                GLOBAL.baseline_mean, self.baseline_std, self.trace_length
            )  # or create mock gauss. baseline
    ...
    def physics_test(self, n_showers: int = None, save_dir: str = None) -> None:
        ...
        ax0.hist(
            all_energy,
            histtype="step",
            bins=np.geomspace(10**16, 10**19.5, 100),
            label="All showers",
        )
        ax0.hist(
            sel_energy,
        ...
        ax3.hist(
            all_spd, histtype="step", bins=np.linspace(0, 6e3, 100), label="All showers"
        )
        ax3.hist(
            sel_spd,
            histtype="step",
            bins=np.linspace(0, 6e3, 100),
            label="Selected showers",
            ls="--",
        )
        ...
        self.ignore_particles and ax4.axvline(
            self.ignore_particles + 1, ls="--", color="gray"
        )
        ax4.hist(
            all_muons,
            histtype="step",
        ...
        ax4.hist(
            sel_muons, histtype="step", bins=particle_bins, color="steelblue", ls="--"
        )
        ax4.hist(
            sel_electrons, histtype="step", bins=particle_bins, color="orange", ls="--"
        )
        ax4.hist(
            sel_photons, histtype="step", bins=particle_bins, color="green", ls="--"
        )
    ...
    def training_test(self, n_showers: int = None) -> None:
        ...
        print(
            f"Cut eliminates {(INT + PAR)/(BKG + SIG + INT + PAR) * 100:.0f}% of all traces\n"
        )
