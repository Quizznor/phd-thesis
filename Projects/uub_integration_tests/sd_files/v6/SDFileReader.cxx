...
int main(int argc, char *argv[]) 
{
  ...
  for (pos = input.FirstEvent(); pos < input.LastEvent(); pos = input.NextEvent())
  {
    ...
    for (const IoSdStation& station : stations)
    {
      ...
      for (int i=0; i < 3; i++) std::cout << "DADt" << i << " " << stationCalib->DADt[i] << "\n";
      for (int i=0; i < 3; i++) std::cout << "SigmaDADt" << i << " " << stationCalib->SigmaDADt[i] << "\n";
      for (int i=0; i < 3; i++) std::cout << "DAChi2" << i << " " << stationCalib->DAChi2[i] << "\n"
    }
  }
  return 0;
}
