...
for file in os.listdir(f"{home_dir}/xy-calibration/config/calib_runlists"):
    df = pd.read_csv(
        f"{home_dir}/xy-calibration/config/calib_runlists/{file}",
        delimiter=";",
        comment="#",
    ...
    for _, run in df.iterrows():
        if not run["DB"]:
            continue
        if "OLO" not in run["src"]:
            continue
        if "reversed positions" in run["comment"]:
            continue
        if run["step"] != 6:
            continue
        if run["runid"] in [
            "14453",  # missing Cal A's
        ...
        exit_code = os.system(
            f"./run_Check.py {run['runid']} -v 0 -s -d {home_dir}/Desktop/xy-quality-files"
        )

        if status.get(run["tel"].strip(), None) is None:
            status[run["tel"].strip()] = [
                [run["date"][:-3].strip(), run["runid"], os.WEXITSTATUS(exit_code)]
            ]
        else:
            status[run["tel"].strip()].append(
                [run["date"][:-3].strip(), run["runid"], os.WEXITSTATUS(exit_code)]
            )
