...
{
  /* add the histogram and start the acquisition
   * h -> the generated histogram.
   * read_buff: used to set trigger condition for the
   *    SSD.
   */
  if(str->file==NULL){
  ...
  if(str->finish==0){
    ...
    muon_read_set2(read_buff,th,trigEnable,1);
  ...
  
{
  ...
  if((bsize + str->headSize + str->head->size < str->nextBlockSize)){
	...
	hhisto->pkmin=Offset[3]+str->ssdTh;
	if(hhisto->pkmin<0){
	  hhisto->pkmin=0;
	}
	...
	hhisto->nPkUnder++; //it should never appear.
      } else {
        hhisto->nentries++;
        if(peak[3]<hhisto->pkmax){
	...
	hparam->chbase[3]=charge_bins[3]*Offset[3];
        hparam->Thres=Offset[3] + str->ssdTh;
      }
    ...
    if (Offset[3]+str->ssdTh < peak[3]){
      //The Offset may change a little during acquisition.
      //  for each acquisition type, it consider a fixed Offset value.
      // The number of entries maybe a little different of the registered
      // events. 
      str->head->nentries++;
    }
  }
