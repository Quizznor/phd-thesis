...
with open(working_dir + file,"rb") as binary_file:
    ...
    with open(f"/cr/tempdata01/filip/SSDCalib/RadioCut/{file.replace('.dat','_WCD')}.dat", "x") as _: pass
    with open(f"/cr/tempdata01/filip/SSDCalib/RadioCut/{file.replace('.dat','_SSD')}.dat", "x") as _: pass 
    ...
    # save SSD traces
    with open(f"/cr/tempdata01/filip/SSDCalib/RadioCut/{file.replace('.dat','_SSD')}.dat", "a") as SSD:
        ssd_baseline = int(np.floor(np.mean(first_bins_ssd)))
        for trace in all_ssd_traces:
            SSD.write(f"{ssd_baseline} " + " ".join([str(int(_)) for _ in trace[:RADIO_CUTOFF]]) + "\n")
