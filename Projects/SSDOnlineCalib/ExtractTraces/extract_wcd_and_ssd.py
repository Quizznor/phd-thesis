...
with open(working_dir + file, "rb") as binary_file:
    ...
    while True:
        ...
        for i in range(len(Saturated)):
            ...
            Baseline[i] = (
                read_to_int() >> 4
            )  # baseline is given as baseline * 16 according to Daves' mail
            Peak[i] = (
                read_to_int()
            )  # different from the 215 point something in simulations !!
            Area[i] = read_to_int()
        ...
        for i in range(n_bins):
            ...
            if i == 0:
                first_bins_ssd.append((ssd_pmt >> 16) & 0xFFF)
        ...
        if write_WCD:
            for i in range(len(low_gain)):
                ...
                if do_filtering:
                    pmt = apply_filtering(pmt)
                if do_downsampling:
                    pmt = apply_downsampling(pmt)
                ...
                with open(wcd_file, "a") as WCD:
                    ...
                    WCD.write(
                        " ".join([str(int(_bin - baseline)) for _bin in pmt]) + "\n"
                    )
    ...
    if write_SSD:
        with open(ssd_file, "w+") as SSD:
            ...
            for trace in all_ssd_traces:
                SSD.write(
                    f"{ssd_baseline} " + " ".join([str(int(_)) for _ in trace]) + "\n"
                )
