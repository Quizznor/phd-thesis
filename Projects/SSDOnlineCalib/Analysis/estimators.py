...
def offline_MIP(x_min, x_max, rates, station, ax=None):
    ...
    popt, pcov = curve_fit(
        landau, range(x_min, x_max), residuals, sigma=rates_normalized_error
    )
    ...
    if ax is not None:
        ...
        c = {
            "NuriaJrFilteredDownsampled": "green",
            "NadiaLateFilteredDownsampled": "tomato",
            "NadiaEarlyFilteredDownsampled": "rebeccapurple",
        }
        ax.errorbar(
            range(x_min, x_max),
            residuals,
            yerr=rates_normalized_error,
            lw=2,
            c=c[station],
            label=station,
            **kwargs
