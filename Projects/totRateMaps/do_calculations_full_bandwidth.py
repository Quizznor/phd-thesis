...
try:
    already_calculated = np.loadtxt(
        f"/cr/data01/filip/Data/totRateMap/{STATION}_FBW.txt", usecols=[0, 1]
    )
    for _m, _t in already_calculated:
        if _m == m and _t == t:
            sys.exit(f"calculation already exists for {m, t}")
...
for File in tools.ProgressBar(UubRandom(f"{STATION}", "wcd"), newline=False):
    ...
    for trace, vem_compatibility in zip(traces, vem_peak):
        # filter_and_downsampled = filter_and_downsample(*trace)
        vem_full_bandwidth = vem_compatibility * 1.3
        calibrated = trace / vem_full_bandwidth[:, np.newaxis]
